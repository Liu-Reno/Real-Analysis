...
$$
\mathring{A}\cup  \mathring{B} \subset \mathring{(A\cup B)},\bigcup_{a \in I}\mathring{E_a} \subset \mathring{(\bigcup_{a\in I}E_a )}
$$
至于证明考虑$(0,1]$和$[1,2)$即可
...
\mathring{A}\cap \mathring{B} =  \mathring{(A \cap B)}
$$
...
$\overline{E^c} = E^c \cup \part (E^c) = E^c \cup \part E$且有$E\cap E^c = \varnothing$于是得知$\overline{E} (E \cup \part E)\cap(E^c \cup \part E) = \part E$
